...
export const CountdownTimer = ({ startTime }) => {
  ...
  return (
    <div className="text-2xl text-rose-300 font-bold mt-4">
      {timerComponents.length ? timerComponents : <span>Contest has begun!</span>}
    </div>
  );
};
...
const ContestDescriptionPage = ({ contestId }) => {
  ...
  <div className="min-h-screen bg-black text-rose-100 p-8">
    <div className="max-w-4xl mx-auto">
      <h1 className="text-4xl font-bold text-rose-500 mb-6 shadow-rose-500/50 shadow-sm">{contest.title}</h1>
      ...
      <div className="space-y-6">
        <div className="bg-indigo-950 bg-opacity-30 rounded-lg p-4">
          ...
          <p className="text-rose-200">{contest.description}</p>
        </div>
        <div className="flex justify-between text-rose-300 bg-emerald-950 bg-opacity-30 rounded-lg p-4">
          <div>
            ...
            <p className="text-emerald-200">{new Date(contest.start_time).toLocaleString()}</p>
          </div>
          <div>
            <h3 className="text-xl font-semibold text-emerald-300 mb-2">End Time</h3>
            <p className="text-emerald-200">{new Date(contest.end_time).toLocaleString()}</p>
          </div>
        </div>
      </div>
    </div>
